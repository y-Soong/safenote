      ...
      >
        ...
        <div class="detail-wrapper">
          ...
          <div class="att-summary">
            ...
            <span class="meta-item meta-danger">미이수 {{ notCompletedCount }}명</span>
                ...
                <template v-if="!attendanceList || attendanceList.length === 0">
                  <tr>
                    <td colspan="9" class="edu-grid-empty">
                      출결 명단이 없습니다. (실시간 진행/모바일 앱 이후 채워집니다)
                    </td>
                  </tr>
                </template>
                <template v-else>
                  ...
                  >
                    ...
                    <td>
                      <span class="anomaly-badge" :class="anomalyClass(row.anomalyLevel)">
...
const fnSignature = async (row) => {
  const parts = [];
  if (row.entrySignFileMgmtCd) parts.push("입실 서명: " + row.entrySignFileMgmtCd);
  if (row.exitSignFileMgmtCd) parts.push("종료 서명: " + row.exitSignFileMgmtCd);
