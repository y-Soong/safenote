      ...
      >
        ...
        <div class="detail-wrapper">
          ...
          <div class="user-meta">
            ...
            <span class="type-badge" :class="user.userTypeCd === 'DAILY' ? 'type-daily' : 'type-regular'">
              {{ user.userTypeCd === "DAILY" ? "일용직" : "정규직" }}
            </span>
            <span class="meta-item">사업장: {{ user.siteNm || user.siteCd || "-" }}</span>
          ...
          <div class="stat-strip">
            ...
            <div class="stat-card">
              ...
              <span class="stat-value">
                {{ summary.avgDurationMin != null ? summary.avgDurationMin + "분" : "-" }}
              </span>
            </div>
          </div>
          ...
          <div class="att-filter no-print">
            ...
            <button class="btn btn-primary btn-sm" @click="fnReload">조회</button>
          </div>
                  ...
                  <tr v-for="(row, idx) in attendances" :key="row.attendanceCd">
                    ...
                    <td style="text-align: center">
                      <span
                        ...
                        "
                      >
                        {{ row.completionStatusNm || compMark(row.completionStatusCd) }}
                      </span>
                    </td>
                  </tr>
                </template>
...
const fnSearch = async () => {
  try {
    ...
    if (response.status === 200) {
      ...
      summary.avgDurationMin = s.avgDurationMin != null ? s.avgDurationMin : null;
    }
  } catch (err) {
    attendances.value = [];
    totalCount.value = 0;
