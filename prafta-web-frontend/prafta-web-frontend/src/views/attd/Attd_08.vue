        ...
        <table class="a08-table">
          ...
          <tbody>
            ...
            >
              ...
              <td>{{ r._isOt ? "-" : dCell(r._inDate) }}</td>
              <td>{{ r._isOt ? "-" : tCell(r._inStdTime) }}</td>
              <td>{{ r._isOt ? "-" : dCell(r._outDate) }}</td>
              <td>{{ r._isOt ? "-" : tCell(r._outStdTime) }}</td>
...
const fnExcel = async () => {
  ...
  const data = displayRows.value.map((r) => [
    ...
    dCell(r._outDate),
    tCell(r._outTime),
    r._isOt ? "-" : dCell(r._inDate),
    r._isOt ? "-" : tCell(r._inStdTime),
    r._isOt ? "-" : dCell(r._outDate),
    r._isOt ? "-" : tCell(r._outStdTime),
...
const displayRows = computed(() =>
  rows.value.map((r) => {
    ...
    return {
      ...
      _inDate: isSeq2 ? r.act2InDate : r.act1InDate,
      _inTime: isSeq2 ? r.act2InTime : r.act1InTime,
      _outDate: isSeq2 ? r.act2OutDate : r.act1OutDate,
      _outTime: isSeq2 ? r.act2OutTime : r.act1OutTime,
      _inStdTime: isSeq2 ? r.act2InStdTime : r.act1InStdTime,
      _outStdTime: isSeq2 ? r.act2OutStdTime : r.act1OutStdTime,
      _rowKey: "a-" + r.attdId,
    };
  })
);
