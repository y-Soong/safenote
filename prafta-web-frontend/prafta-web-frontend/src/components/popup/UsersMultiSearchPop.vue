            ...
            <table class="data-grid">
              ...
              <tbody>
                <tr
                  ...
                  :key="user.userCd"
                  :class="{ selected: selectedMap[user.userCd] }"
                  @click="toggleRow(user.userCd)"
                >
                  <td class="check-col">
                    <input
                      type="checkbox"
                      :checked="!!selectedMap[user.userCd]"
                      @click.stop
                      @change="toggleRow(user.userCd)"
                    />
                  </td>
                  <td>{{ user.userId }}</td>
                  <td>{{ user.userNm }}</td>
...
const isAllChecked = computed(
  () =>
    userActList.value.length > 0 &&
    selectedCount.value === userActList.value.length
);
const isIndeterminate = computed(
  () =>
    selectedCount.value > 0 && selectedCount.value < userActList.value.length
);
...
const fnGetUserInfoList = async () => {
  ...
  let apiUrl = "/comApi/baseinfo/user-info-lists";
  let params = {
    userId: userId.value,
    userNm: userNm.value,
    searchMode: "SHIFT_USER_SRCH",
  };
...
const toggleRow = (userCd) => {
  if (selectedMap[userCd]) delete selectedMap[userCd];
  else selectedMap[userCd] = true;
};
...
const toggleAll = () => {
  if (isAllChecked.value) {
    ...
    userActList.value.forEach((u) => {
      selectedMap[u.userCd] = true;
    });
  }
};

const fnConfirm = () => {
  const selected = userActList.value.filter((u) => selectedMap[u.userCd]);
  if (selected.length === 0) {
    fnAlertMsg("선택된 사용자가 없습니다.");
    return;
  }
