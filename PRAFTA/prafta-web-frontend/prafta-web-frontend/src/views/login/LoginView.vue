...
*/
  try {
    ...
    if (response.status === 200) {
      // PRAFTA-036: 인증대기 분기. 관리자가 만든 계정의 첫 로그인은 휴대폰 본인인증 후에만 진행한다.
...
/**
 * 정식 LoginResponse 를 sessionStorage / userStore 에 적용하고 메인 라우팅까지 이어준다.
...
const fnApplyLoginResponse = (data) => {
  ...
  sessionStorage.setItem("gv_userId", respUserId);
  ...
  if (rememberId.value) localStorage.setItem("savedUserId", respUserId);
