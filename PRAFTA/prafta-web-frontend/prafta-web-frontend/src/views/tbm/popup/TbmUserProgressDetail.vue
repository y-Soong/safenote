      ...
      >
        ...
        <div class="detail-wrapper">
          ...
          <div class="user-meta">
            ...
            >
              {{
                user.userTypeCd === "DAILY"
                  ? "일용직"
                  : user.employmentTypeNm || "정규직"
              }}
            </span>
          ...
          <div class="stat-strip">
            <div class="stat-card">
              <span class="stat-label">누적 교육시간</span>
              <span class="stat-value">
                {{ fmtMinutes(summary.totalEduMinutes) }}
              </span>
            </div>
            <div class="stat-card">
              <span class="stat-label">수료</span>
            ...
            <table class="data-grid w-full text-sm text-left">
              <thead>
                <tr>
                  ...
                  <th style="width: 13%">종료시각</th>
                  <th style="width: 9%; text-align: center">이수상태</th>
                </tr>
              </thead>
              <tbody>
...
// 사용자별 세션 이수 이력 조회(GET /webApi/tbm03/user-progress-detail)
const fnSearch = async () => {
  try {
    const response = await axios.get("/webApi/tbm03/user-progress-detail", {
      params: {
    ...
    if (response.status === 200) {
      ...
      user.userNm = u.userNm || props.userNm_p || "";
      user.employmentTypeNm = u.employmentTypeNm || "";
      ...
      summary.totalEduMinutes = s.totalEduMinutes || 0;
    ...
    attendances.value = [];
    totalCount.value = 0;
    summary.totalEduMinutes = 0;
    summary.completedCount = 0;
    summary.notCompletedCount = 0;
    await proxy.$alert(
...
// 인정시간(분) 표시 포맷: 0/NULL→"0분", <60→"N분", >=60→"h시간 m분"(m=0이면 "h시간")
