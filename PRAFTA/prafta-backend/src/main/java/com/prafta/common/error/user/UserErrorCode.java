...
public enum UserErrorCode implements ApiErrorCode {
    ...
    // ===== 소속이동 기본 근무타입 적용일 검증(2026-08-22, 스케줄러 검증 중 발견) =====
    // 지정한 근무타입이 이동 사업장에 활성으로 존재하지 않거나, 최초 적용일자가 이동일보다 미래인 경우.
    //   (적용일 이전 구간은 유효버전 폴백으로 시각이 해석돼 오류는 없지만, 적용일자 의미와 어긋난
    //    예약을 등록 시점에 fail-closed 로 차단한다. 셀렉트 필터와 별개로 서버가 최종 판정.)
    , USER_400_083(HttpStatus.BAD_REQUEST, "이동일에 적용할 수 없는 근무타입입니다.\n적용일자가 이동일 이전인 근무타입을 선택해 주세요.")
    ;
