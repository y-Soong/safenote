...
public enum AttdErrorCode implements ApiErrorCode {
    ...
    // 반차·반반차·시간차 연차는 근무 스케줄(소정근로시간) 기준으로 차감하므로, 스케줄이 등록된 날에만 신청 가능.
    //   종일(00) 연차는 항상 1일 차감이라 스케줄이 없어도 신청 가능(이 가드 비대상). (LC-06: 반반차 '05' 포함)
    , ATTD_400_110(HttpStatus.BAD_REQUEST, "반차·반반차·시간차 연차는 근무 스케줄이 등록된 날에만 사용할 수 있어요. 스케줄이 없으면 종일 연차로 신청해 주세요.")
