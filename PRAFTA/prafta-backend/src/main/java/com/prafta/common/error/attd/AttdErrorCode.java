...
public enum AttdErrorCode implements ApiErrorCode {
    ...
    // ===== 접수함 "연차 변경 요청 대기" 다중 사업장·부서 확장 (2026-08-23) =====
    // §0-5 설계 결정: 부서(NODE_CD) 필터는 접근 가능 사업장이 정확히 1건으로 좁혀졌을 때만 유효.
    //   다건(전체 사업장 등)에 걸쳐 동일 NODE_CD 로 매칭하는 것은 사업장 간 NODE_CD 재사용
    //   함정(feedback_web_new_query_screen_needs_node_gate 계열) 때문에 채택하지 않는다.
    //   종전엔 이 조합이 조용히 빈 결과를 반환했으나(WHERE NU.SITE_CD = NULL), 접수함 관례상
    //   fail-visible 하게 명확한 에러로 거부한다.
    , ATTD_400_214(HttpStatus.BAD_REQUEST,
            "부서 필터는 사업장을 1곳 선택했을 때만 사용할 수 있습니다.")

    // 연차 신청 증빙 필수화(2026-08-29): EVIDENCE_YN='Y' 타입인데 증빙 파일 미첨부로 신청 거부.
    , ATTD_400_215(HttpStatus.BAD_REQUEST,
            "증빙 자료 첨부가 필요한 연차입니다. 파일을 첨부한 뒤 다시 신청해 주세요.")

    // sec 리뷰(Critical, 2026-08-29): evidenceFileId 가 존재하지 않거나, FILE_TYPE='008'이 아니거나,
    //   업로더(본인)가 아닌 경우 — 가짜/타인 파일 ID 로 신청되는 것을 차단.
    , ATTD_400_216(HttpStatus.BAD_REQUEST,
            "존재하지 않거나 본인이 업로드하지 않은 증빙 파일입니다. 다시 첨부한 뒤 신청해 주세요.")

    // ===== 부분휴가(반차·시간차) 휴게 무시 도입(BW-04, 2026-09-04) — 근기법 제54조① 단서 =====
    // 회사 연차 사용정책 BRK_WAIVE_ALLOW_YN='N'(또는 활성 정책 없음 — fail-closed)인데 brkWaiveYn='Y' 전달.
    , ATTD_400_217(HttpStatus.BAD_REQUEST,
            "이 회사는 휴게시간 무시 요청을 허용하지 않습니다. 관리자에게 문의해 주세요.")
    // 관리자 직접 차감(근무계획 저장/엑셀) 경로에 brkWaiveYn='Y' 가 실려 온 경우 — 근로자 본인 앱/웹 신청 경로 한정.
    , ATTD_400_218(HttpStatus.BAD_REQUEST,
            "휴게시간 무시 요청은 근로자 본인이 앱에서 신청할 때만 선택할 수 있습니다.")
    // 종일(00)·기간 신청 등 반차(01)/시간차(02/03/04) 외 단위에 brkWaiveYn='Y' 전달.
    , ATTD_400_219(HttpStatus.BAD_REQUEST,
            "휴게시간 무시는 반차·시간차 연차에서만 선택할 수 있습니다.")

    // ===== §7-1 단시간 근로자 휴게 미이용 "상시" 요청(BW-12, 2026-09-04) =====
    // 마이페이지 토글 저장 요청의 standingYn 이 'Y'/'N' 이 아니거나, 대상 아닌 사용자(일용직 등)가 호출한 경우.
    // v2(2026-09-05, 법정 하한 상한제 §7 Q6): 상시 요청 폐지 — 결번 유지(삭제 금지, 신규 사용 금지).
    , @Deprecated
    ATTD_400_220(HttpStatus.BAD_REQUEST,
            "휴게 미이용 상시 요청을 저장할 수 없습니다. 값을 확인한 뒤 다시 시도해 주세요.")

    // ===== 부분휴가 휴게 넘김 v2 — 법정 하한 상한제(BW2-04·05, 2026-09-05) =====
    // brkWaiveMin 이 음수/15분 배수 아님, 시간차·종일에 분량(>0) 전송, 반차 실효 cap(min(법정 cap, 파트 movable)) 초과.
    , ATTD_400_221(HttpStatus.BAD_REQUEST,
            "넘길 휴게 분량이 올바르지 않습니다. 허용 범위(15분 단위, 최대치)를 확인한 뒤 다시 선택해 주세요.")
    // 시간차 휴게 편입(brkWaiveYn='Y') 후 남는 회사 휴게(B − absorbed)가 법정 하한(req) 미만 — R4 거부.
    , ATTD_400_222(HttpStatus.BAD_REQUEST,
            "휴게를 넘기면 이 날 법정 휴게 시간이 부족해 신청할 수 없습니다. 휴게 넘김 없이 신청하거나 시간대를 바꿔 주세요.")
