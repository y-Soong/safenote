...
/**
 * 대시보드 근태 탭 A1 근무계획 등록율 조회 파라미터 (PRAFTA-DASHBOARD-T2).
 * siteCd/workYm 필수 + workYm 형식(YYYY-MM) 검증, incSubNodeYn 은 'Y' 외 전부 'N' 정규화.
 */
public record DashAttdPlanRegRateParam(
    String siteCd
    , String nodeCd
    , String incSubNodeYn
    , String workYm
    ...
    public static DashAttdPlanRegRateParam from(DashAttdPlanRegRateRequest request, TokenInfo tokenInfo) {
        ...
        return new DashAttdPlanRegRateParam(
            request.getSiteCd()
            , request.getNodeCd()
