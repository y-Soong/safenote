package com.prafta.web.notice.notice01.result;

import java.util.List;

import com.fasterxml.jackson.annotation.JsonProperty;

import lombok.Builder;
import lombok.Getter;

/**
 * 로그인 팝업 노출 공지 1건(첨부 목록 포함) 응답 아이템.
 * 매퍼 결과(NoticePopupResult)에 첨부 list(NoticeFileResult)를 서비스에서 합성한다.
 * 팝업에서 첨부를 바로 다운로드할 수 있도록 fileMgmtCd/fileNm 을 내려준다(다운로드는 단기 토큰 흐름 재사용).
 * boolean is- 접두 Jackson 탈락 방지로 @JsonProperty 고정.
 */
@Getter
@Builder
public class NoticePopupItem {
    private String noticeId;
    private String title;
    private String content;
    private String pinYn;
    private String popupFromYmd;
    private String popupToYmd;
    private String insertDate;
    private Integer fileCnt;
    @JsonProperty("isDaily")
    private boolean isDaily;
    private List<NoticeFileResult> fileList;
}
