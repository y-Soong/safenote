 ...
 * 정책 §11.1 민감항목(휴대폰·이메일·생년월일)은 대기 목록({@link SelfJoinListResponse})과
 * <b>동일 집합</b>으로 고정한다 — 이력이라는 이유로 늘리지 않는다.
 * 대기 목록에 없는 항목은 <b>처리 결과 표시에 필요한 입사일·직급명·처리자명뿐</b>이며(지시서 §3),
 * 셋 다 민감항목이 아니다. 권한·고용형태는 전 행 동일값이라 추가하지 않는다.
 *
...
public class SelfJoinHistoryListResponse {
    ...
    public static class Row {
         ...
         *
         * <p>거부 건은 <b>항상 null</b> 이다 — 매퍼가 action='APPROVE' 로 가드한다.
         * 가드가 없으면 거부→재가입→승인 경로에서 거부 행에 나중 승인의 입사일이 실린다.
         */
        String hireDate;

        /** 승인 당시 직급명. 미지정이면 null, 거부 건은 <b>항상 null</b>(매퍼 action 가드). */
        String rankNm;
