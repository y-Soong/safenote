...
--   개발 DB : ✅ 이미 적용됨 — 리허설(MyBatis 검증) 목적으로 선적용했다.
--             ★다시 실행하면 "Error 1060. Duplicate column name" 이 난다. 정상이며, 건너뛰면 된다.
--   운영 DB : ❌ 미적용 — §2 를 실행할 곳은 여기다.
--
-- ★애플리케이션 배포보다 먼저 적용한다(신규 컬럼을 읽는 조회 쿼리가 배포와 함께 나간다).
-- ★멱등하지 않다(MySQL 8.0 은 ADD COLUMN IF NOT EXISTS 미지원).
--   반드시 §1 을 먼저 실행해 결과가 0행일 때만 §2 로 넘어간다.
-- =====================================================================================
...
-- =====================================================================================
-- §1. 사전 확인 — ★반드시 먼저 실행할 것
--
--   결과 0행 → §2 로 진행(미적용 환경)
--   결과 3행 → 이미 적용된 환경. §2 를 건너뛰고 §3 검증만 실행한다.
--              (개발 DB 가 여기 해당한다 — 억지로 실행하면 1060 오류)
-- =====================================================================================
