<!--
  ...
  - 노출 조건: 개설된 교육(OPENED/IN_PROGRESS) 세션 상세에서만(요청서). pwdVisible(서버 산출) 기준.
  - 재발급: OPENED 상태에서만(web Tbm_02 규칙). POST .../regenerate-passwords 는 부모/developer 가 호출.
...
<template>
  <section class="admin-tbm-pwd">
    <p class="admin-tbm-pwd__title">입실 / 종료 비밀번호</p>

    <div class="admin-tbm-pwd__row">
      <div class="admin-tbm-pwd__box">
    ...
    <p class="admin-tbm-pwd__hint">근로자에게 입실/종료 비밀번호를 안내해 주세요.</p>
  </section>
</template>
...
defineProps({
  ...
  // 재발급 가능 여부(OPENED 한정 — 부모가 statusCd 로 판정해 전달)
  canRegenerate: { type: Boolean, default: false },
  // 재발급 진행 중 버튼 잠금
  regenerating: { type: Boolean, default: false },
})

// regenerate → 부모가 POST .../regenerate-passwords 호출 후 entryPwd/exitPwd 갱신
