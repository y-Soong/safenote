<template>
  <div class="min-h-screen flex flex-col bg-white">
    <!-- 상단 헤더 (화면명 표시) — NearMissReport 헤더와 동일 UI로 일관성 유지.
         Tailwind v2(JIT 미사용)라 임의값 클래스를 못 쓰므로 scoped CSS(.risk-hd)로 구현. -->
    <header class="risk-hd">
      <button type="button" class="risk-hd__back" aria-label="뒤로" @click="goBack">
        <svg
      ...
      <h1 class="risk-hd__title">위험성 발굴</h1>
      <span class="risk-hd__spacer" aria-hidden="true"></span>
    </header>
...
<style scoped>
/* 상단 헤더 (화면명 표시) — NearMissReport(.nmr-hd)와 동일 UI.
   토큰 미선언 화면이라 색상은 토큰값과 동일한 리터럴로 지정.
   padding-top: max(env(safe-area-inset-top), 12px) — 앱 webview(env=0)에서도 최소 상단 여백 보장. */
.risk-hd {
  min-height: 56px;
  flex-shrink: 0;
  background: #ffffff;
