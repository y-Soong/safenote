<template>
  <div class="min-h-screen flex flex-col bg-white">
    ...
    <div class="p-6">
      <button
        @click="fnUserPwReset"
        class="w-full mb-4 bg-white text-green-600 border border-green-600 py-3 rounded-md hover:bg-green-50 transition"
        v-if="userIdShowFlg"
      >
...
const cmpnyCd = ref('')
const userId = ref('')
// prafta-036-A: 비밀번호 재설정 시 userCd가 필요 → fnUserIdSrch 응답에서 저장
const userCd = ref('')
const userNm = ref('')
...
const fnUserIdSrch = async () => {
  try {
    ...
    if (response.status === 200) {
      // prafta-036-A: record 래핑(userIdInfoResult) + camelCase, userCd도 저장 (#13에서 사용)
      const result = response.data?.userIdInfoResult || {}
      userId.value = result.userId || ''
      cmpnyCd.value = result.cmpnyCd || ''
      userCd.value = result.userCd || ''
      fnSmsAuthReqApi()
    }
  } catch (err) {
    // prafta-036-A: 옵셔널 체이닝으로 NPE 안전화
    fnAlertMsg(err.response?.data?.message || '계정 정보를 조회할 수 없습니다.')
  }
}
...
const fnUserPwUpdate = async () => {
  try {
    // prafta-036-A: 백엔드 케밥 정렬에 맞춰 URL 변경 + 식별자 userId → userCd (UserPasswordRequest 시그니처 일치)
    ...
    // prafta-036-A: 옵셔널 체이닝으로 NPE 안전화
    fnAlertMsg(err.response?.data?.message || '비밀번호 재설정에 실패했습니다.', () => {
