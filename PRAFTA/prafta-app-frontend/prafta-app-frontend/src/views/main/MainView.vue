...
<template>
  <div class="home-view">
    ...
    >
      ...
      <!-- 당겨서 새로고침 인디케이터 — 스크롤 최하단에서 위로 더 당기면 노출 -->
      <div
        class="pull-refresh"
      ...
      >
        ...
        <span v-else-if="pullDistance > 0" class="pull-refresh__text">위로 당겨 새로고침</span>
      </div>
    </main>
...
// ───────────────────────────────────────────────────────────
// 당겨서 새로고침 — 스크롤 최하단에서 위로 더 당기면(overscroll) home-summary 재조회.
//   1) touchstart 시점에 스크롤이 최하단이면 추적 시작
//   2) touchmove 에서 위로 당긴 거리(저항감 0.5배)를 인디케이터 높이로 환산
...
const onPullStart = (e) => {
  ...
  // 최하단에 있을 때만 당김 추적 시작
  tracking = isScrolledToBottom()
  if (tracking) touchStartY = e.touches[0].clientY
}

const onPullMove = (e) => {
  if (!tracking || isRefreshing.value) return
  const delta = touchStartY - e.touches[0].clientY // 위로 당기면 양수
...
// ───────────────────────────────────────────────────────────
// prafta-app-008: 외근 사유 시트 + §5.5 Case C 2-pass 출퇴근 흐름 (MyAttendanceView 와 동형)
...
//   (홈 화면은 today 미보유 → workYmd 생략, 서버가 최신 열린 근태로 폴백.)
// ───────────────────────────────────────────────────────────
...
//   mode: 'checkIn'|'checkOut', ctx: { lat, lon, accuracy, isMocked, offsiteReason, confirmSkipPrevSlot }
