 ...
 * (web 프론트 `@/utils/validationNotice` 와 동일 시그니처·동일 문구 규격)
 *
 * ─────────────────────────────────────────────────────────────────────────────
 ...
 * 기준 사용처(웹): `attd/popup/SchInfoPop.vue` fnSave.
 *
 * ─────────────────────────────────────────────────────────────────────────────
 ...
 *  - 앱은 전역 래퍼 관례가 없고 화면마다 `showAlert`/`showConfirm`/`askConfirm` 을 재선언한다
 *    (`proxy?.$alert ? ... : window.alert` 폴백 분기 포함). 전역을 가정하면 웹뷰 외 실행
 *    환경에서 깨진다. 주입형이면 그 래퍼를 그대로 넘기면 된다(래퍼 이름 통일 불필요).
 ...
 *   if (!(await notices.resolve('저장하시겠습니까?'))) return
 *
 * ─────────────────────────────────────────────────────────────────────────────
 * 문구 규격
 * ─────────────────────────────────────────────────────────────────────────────
 ...
 * }} io 표시 함수 주입(앱: 뷰의 로컬 showAlert / askConfirm 래퍼)
 * @returns {{
...
export function createNotices(io = {}) {
  ...
  const api = {
    ...
    resolve: async (baseConfirmMessage, options = {}) => {
      ...
      const message = buildNoticeMessage(notices, baseConfirmMessage, options)
