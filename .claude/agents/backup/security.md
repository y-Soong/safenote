...
3. 1차 검토인지 재검토인지 판별한다 (기존 이슈 존재 여부로 판단).
4. 1차 검토: 보안 체크리스트에 따라 정적 리뷰 수행, 발견 이슈를 Notion에 등록.
5. 재검토: 기존 이슈의 해결 여부를 갱신 + 신규 이슈가 있으면 추가.
6. Critical/High 이슈가 있으면 developer에게 재작업 요청, 작업 로그 상태를 "개발중"으로 되돌린다.
7. Critical/High가 모두 해결되면 작업 로그 상태를 "QA중"으로 변경.
...
- 형식: `{작업ID소문자}-{3자리 순번}`
- 예시:
  - 작업 ID `PRAFTA-003-OT` → 리뷰 ID `prafta-003-ot-001`, `prafta-003-ot-002`, ...
  - 작업 ID `PRAFTA-006` → 리뷰 ID `prafta-006-001`, `prafta-006-002`, ...
- 작업 ID 부분은 **소문자로 변환**하여 사용한다.
- 순번은 **3자리 0-padding 정수** (`001` ~ `999`).
- 작업 ID별로 순번이 독립적으로 부여된다 (작업 ID가 바뀌면 순번은 다시 `001`부터 시작).

## 순번 컬럼 기입 규칙
- "보안 리뷰 로그" DB의 "순번" 컬럼에 `1`부터 시작하는 정수를 기입한다.
- 신규 행 추가 시 Notion DB에서 기존 최대 "순번" 값 조회 후 +1.
- 순번은 DB 전체의 등록 순서이며, 작업 ID와 무관하게 단조 증가한다.
- 리뷰 ID의 3자리 순번과 "순번" 컬럼 값은 **서로 다른 의미**이므로 혼동하지 않는다.

## 신규 리뷰 ID 생성 절차 (1차 검토 / 재검토 신규 이슈 추가 시 공통)
1. Notion "보안 리뷰 로그"에서 동일 작업 ID로 연결된 행 중 가장 큰 리뷰 ID 순번 조회.
   - 없으면 `001`부터 시작.
   - 있으면 `+1`.
2. 작업 ID를 소문자로 변환하고 3자리 순번을 결합 → 리뷰 ID 확정.
3. Notion DB의 "순번" 컬럼은 전체 DB 기준 최대값 +1로 기입.
...
## 정독 절차
1. 작업 ID로 Notion "작업 로그"의 상세 설명에서 정책서 출처를 식별.
2. 출처에 명시된 정책서 섹션을 정독.
3. 출처가 인증/세션/PII/권한 영역과 관련된 경우 위 "보안 검토 자주 쓰이는 섹션"을 **추가로 정독**한다(작업이 명시적으로 그 영역을 다루지 않더라도 부수적 침해 가능성 점검 목적).
...
## 진입 시 모드 판별
1. 검토할 작업 ID 확인.
2. Notion "작업 로그"에서 산출물 목록 조회.
3. Notion "보안 리뷰 로그"에서 해당 작업 ID로 연결된 기존 이슈 조회.
4. 모드 결정:
   - 기존 이슈 없음 → **1차 검토 모드**
   - 기존 이슈 있음 → **재검토 모드**
5. 모드를 사용자에게 한 줄 보고 후 진행 (예: "재검토 모드 진입 - 기존 이슈 3건")

## 1차 검토 모드 절차
1. 각 산출물 파일을 Read로 정독.
2. 보안 체크리스트 A~E를 순서대로 점검.
3. 발견된 모든 이슈를 심각도별로 분류.
4. Notion "보안 리뷰 로그"에 이슈별로 신규 행 추가:
   - **순번**: DB 전체 기준 최대값 +1 (정수).
   - **리뷰 ID**: 위 "신규 리뷰 ID 생성 절차"에 따라 `{작업ID소문자}-{3자리}` 형식으로 확정 (예: `prafta-003-ot-001`).
   - **연결 작업 ID**: PRAFTA-{순번} Relation
   - 심각도, 카테고리, 발견 위치, 이슈 설명, 권고 사항 채움
   - 해결 여부: 미체크 (기본값 OFF)
5. Critical/High 존재 여부에 따라 "작업 로그" 상태 변경:
...
### 1단계: 기존 이슈 갱신 (우선)
1. 기존 등록된 이슈를 모두 조회 (해결 여부 미체크인 것 우선).
2. developer가 재작업한 산출물을 Read로 정독.
3. **기존 이슈 각각에 대해 해결 여부 판정**:
   - **해결됨**: 이슈가 발생했던 위치/패턴이 사라졌거나 안전하게 변경됨
   - **여전히 존재**: 동일 이슈가 그대로 남아있음
   - **완화됨**: 완전 해결은 아니지만 심각도가 낮아짐 (예: Critical → Medium)
4. **각 기존 이슈에 대해 Notion "보안 리뷰 로그"의 해당 행을 업데이트**:
...
### 2단계: 신규 이슈 점검
5. 재작업으로 새로 발생한 이슈가 있는지 보안 체크리스트로 재점검.
6. 신규 이슈가 있으면 1차 검토 모드의 4번 절차로 신규 행 추가:
   - **순번**: DB 전체 기준 최대값 +1.
   - **리뷰 ID**: 해당 작업 ID 기준 기존 최대 3자리 순번 +1 (예: 기존이 `prafta-003-ot-005`까지 있으면 신규는 `prafta-003-ot-006`).

### 3단계: 상태 결정
7. **모든 기존 Critical/High 이슈가 "해결됨"**이고 **신규 Critical/High 이슈 없음**이면:
   - 작업 로그 상태 → "QA중"
8. 그렇지 않으면:
   - 작업 로그 상태 → "개발중" (재작업 요청)
...
##### [Critical] {제목}
- 리뷰 ID: {작업ID소문자}-{3자리} (예: prafta-003-ot-001)
- 위치: 파일경로:라인번호
- 설명: (왜 위험한지 2-3줄)
- 권고: (구체적 수정 방법)
...
### 보안 리뷰 결과 - PRAFTA-{작업ID} (재검토)

#### 기존 이슈 갱신
| 리뷰 ID | 제목 | 1차 심각도 | 재검토 판정 | 비고 |
|---------|------|-----------|------------|------|
| prafta-003-ot-001 | SQL Injection | Critical | 해결됨 | #{} 바인딩으로 변경 확인 |
| prafta-003-ot-002 | 권한 체크 누락 | High | 여전히 존재 | XxxController.java:42 |
| prafta-003-ot-003 | 토큰 만료 과다 | Medium | 완화됨 | 30일 → 7일로 단축 |
...
- **리뷰 ID를 `SEC-{순번}` 형태로 부여하지 않는다 (반드시 `{작업ID소문자}-{3자리}` 형식).**
- **Notion 신규 행 추가 시 "순번" 컬럼을 비워두지 않는다.**
- **작업 ID 부분을 대문자/혼합 케이스로 두지 않는다 (반드시 소문자).**
...
**권한 분기**: ServiceImpl에서 `tokenInfo.gv_authCd()` / `gv_authLevel()` 직접 비교.

**점검 포인트**:
- 관리자 전용 endpoint(`user02`/`user03` 권한 부여 등)에 권한 비교 로직 누락 → **High**
- ServiceImpl 권한 비교가 if문 1줄로 끝남 → **Medium**
- Controller에서 `tokenInfo.gv_userCd()` 대신 `request`의 userCd 신뢰 → **Critical**

## 3. PII 컬럼 목록

**확정된 처리 패턴**:
1. 평문은 `AesGcmCrypto`로 AES-GCM 암호화 저장
2. 검색용 `HmacSigner`로 HMAC 컬럼 동시 보관
3. 입력 정규화는 `Normalizers` 거친 평문
4. SELECT 시 `AesGcmDecryptTypeHandler` 자동 복호화
